...
def sacar_citas():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("SELECT id_m FROM medicos WHERE especialidad=%s", [medico])
        medico_id = cur.fetchone()[0]

        cur.execute("SELECT id_p FROM pacientes WHERE correo=%s", [correo])
        paciente_id = cur.fetchone()[0]

        cur.execute("INSERT INTO citas (nombre, telefono, correo, sintomas, fecha, medico) VALUES (%s, %s, %s, %s, %s, %s)", (nombre, telefono, correo, sintomas, fecha, medico))
        mysql.connection.commit()
    ...
    cur = mysql.connection.cursor()
    cur.execute("SELECT especialidad FROM medicos")
    especialidades = cur.fetchall()
    cur.close()
    return render_template("sacar_citas.html", especialidades=especialidades)
